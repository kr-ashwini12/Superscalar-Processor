...
module dispatch #(
    ...
    // ----------------------------------------------------------
    wire A_dispatches = decode_A_valid_i & ~rename_stall_A_i & ~rob_full_i;

    // B only dispatches if A also dispatches this cycle
    wire B_dispatches = decode_B_valid_i & ~rename_stall_B_i & ~rob_full_i
                        & A_dispatches;

    // Stall fetch if any valid slot can't dispatch
    ...
    // ----------------------------------------------------------
    // RS dispatch outputs
    // Only send to RS if ALU instruction (fu_type == 0)
    // LSU and branches: dispatch to ROB only (stub for Week 6)
    // ----------------------------------------------------------
    ...
    // B gets rob_tail + 1 if A also dispatched
    assign rs_disp_B_rob_idx_o = rob_tail_i + (A_dispatches ? {{(ROB_ADDR_W-1){1'b0}}, 1'b1} : {ROB_ADDR_W{1'b0}});
